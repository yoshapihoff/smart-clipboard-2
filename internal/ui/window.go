...
// Функция для создания системного трея с меню
func SetupSystemTray(app *App, iconResource *fyne.StaticResource) {
	if desk, ok := app.App.(desktop.App); ok {
		...
		// Создаем только наши пункты меню
		settingsItem := fyne.NewMenuItem("Settings", func() {
			app.ToggleWindow()
		})

		// Создаем меню только с нашими пунктами и пустым заголовком
		menu := fyne.NewMenu("")
		menu.Items = []*fyne.MenuItem{settingsItem}
